function[Lx, Lz, pho_h, Ptx, delta_psi, delta_theta, SNR_rx_dB] = Ant_design(pho_x, pho_gr, g_swath, H, theta0)
...
%delta_theta = 2*asin((g_swath*sin(2*theta0))/(4*H)); % calculate from ground swath [rad]

%delta_theta_eq = @(del_theta) del_theta - (asin( g_swath*sin(pi/2 + theta0)/(H*cos(theta0+del_theta/2))));
delta_theta_eq = @(del_theta) (g_swath - 2*H*sin(del_theta)) / (cos(2*theta0) + cos(del_theta));

options_f = optimoptions ('fsolve', 'Display', 'none');
delta_theta = fsolve(delta_theta_eq,0,options_f);


%delta_theta = delta_theta*sign(theta0);

Lz = lambda/delta_theta; % physical antenna width [m] 
%format shortG
%debug = [rad2deg(delta_theta) Lx Lz g_swath H]
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
y_min = H*tan(theta0-(delta_theta/2));
%y_max = y_med + g_swath/2;
y_max = H*tan(theta0+(delta_theta/2));
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
T0_ref = 94; % Reference Temperature [K] % Titan
...
Prx = Ptx*G_ant*A_e*(f_rx^2)*sigma_0*pho_x*pho_r/(sin(theta0)*(4*pi*r_max^2)^2); % Reciever Power
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
