...
%% Calculations for SAR Mode only
Tx_DC = 0.2; % Duty Cycle of Tx Antenna
instr_DC = 0.2; % Payload Scan Orbital Duty Cycle
...
num_phase_orbit = (SAR_mission_time/SAR_phases/orb_T);
...
num_Targets = 50;
[Signal2Ambiguity, Duration_acquisition_simulated] = IMG_SIM_SAR(f0, H_min, v_orb, PRF, Tx_DC, Ptx_peak, 10^(G_ref_dB/10), 10^(G_ref_dB/10), sigma_0, L, theta_0, delta_theta, pho_r, num_Targets);
...
writetable(struct2table(Ant.Array), 'SAR_Design.xlsx', 'WriteMode', 'overwritesheet', 'Sheet', 'Array');

Ant.Antenna.Length = Lx;
Ant.Antenna.Width = Lz;
Ant.Antenna.TransmitterPower = Ptx;
Ant.Antenna.PeakTransmitterPower = Ptx_peak;
Ant.Antenna.RecieverPower = Prx;
Ant.Antenna.PeakRecieverPower = Prx_peak;
...
writetable(struct2table(Ant.Antenna), 'SAR_Design.xlsx', 'WriteMode', 'overwritesheet', 'Sheet', 'Antenna');
...
writetable(struct2table(Ant.Reflector), 'SAR_Design.xlsx', 'WriteMode', 'overwritesheet', 'Sheet', 'Reflector');

Ant.Physical.Ant_Mass = Ant_Mass;
Ant.Physical.Ref_Mass = Ref_Mass;
writetable(struct2table(Ant.Physical), 'SAR_Design.xlsx', 'WriteMode', 'overwritesheet', 'Sheet', 'Physical');
...
writetable(struct2table(Ant.Scan), 'SAR_Design.xlsx', 'WriteMode', 'overwritesheet', 'Sheet', 'Scan');

Ant.Res.Altitude = pho_h;
Ant.Res.Azimuth = pho_x;
Ant.Res.GroundRange = pho_gr;
Ant.Res.Range = pho_r;
writetable(struct2table(Ant.Res), 'SAR_Design.xlsx', 'WriteMode', 'overwritesheet', 'Sheet', 'Resolution');

Ant.BW.Elevation = rad2deg(delta_theta);
Ant.BW.Azimuth = rad2deg(delta_psi);
writetable(struct2table(Ant.BW), 'SAR_Design.xlsx', 'WriteMode', 'overwritesheet', 'Sheet', 'BandWidth');

Ant.Pulse.PRI = PRI;
Ant.Pulse.DutyCycle = duty_cycle;
Ant.Pulse.ChirpRate = alfa;
writetable(struct2table(Ant.Pulse), 'SAR_Design.xlsx', 'WriteMode', 'overwritesheet', 'Sheet', 'Pulse');
...
writetable(struct2table(Ant.Noise), 'SAR_Design.xlsx', 'WriteMode', 'overwritesheet', 'Sheet', 'Noise');

Ant.Tx.MinTransmissionSNR_dB = tx_SNR_min;
Ant.Tx.TransmissionWindow = transmission_window;
Ant.Tx.TransmissionOrbits = tx_orb_nos;
writetable(struct2table(Ant.Tx), 'SAR_Design.xlsx', 'WriteMode', 'overwritesheet', 'Sheet', 'DataTransmission');
...
writetable(struct2table(Ant.Mission), 'SAR_Design.xlsx', 'WriteMode', 'overwritesheet', 'Sheet', 'Mission');